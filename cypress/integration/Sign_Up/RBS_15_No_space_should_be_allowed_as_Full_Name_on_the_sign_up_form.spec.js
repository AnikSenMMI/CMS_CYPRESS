...
describe("RBS_15_No_space_should_be_allowed_as_Full_Name_on_the_sign_up_form",()=>{
    afterEach("waiting",() => {
    ...
    it ("input name with space in it", ()=>
    base.writting_on_input(base_locator.first_name, SignUp_page.First_name_with_space)
    )

    it("clicking on Submit button",()=>
    {
        base.clicking_using_xpath(base_locator.submit_button)
    })
    it("assertion--> first name has error in it about space", ()=>
    {
        cy.xpath(base_locator.first_name_space_error).should('contain' , "Spaces are not allowed")
        var text = cy.xpath(base_locator.first_name_space_error).invoke('text')
        cy.log(text)
